...
void InitTetris(){
    ...
    nextBlock[2]=rand()%7;
    init_root();
    modified_recommend(recRoot,recRoot->lv);
...
void BlockDown(int sig){
    ...
    modified_recommend(recRoot,recRoot->lv);
    stop_t=time(NULL);
    duration_t+=(double)difftime(stop, start);
    DrawNextBlock(nextBlock);
    DrawField();
    timed_out=0;
    return;
...
int modified_recommend(RecNode* root, int lv){
    ...
    if(nextBlock[lv]==4) num_r = 1;
    else if (nextBlock[lv]==0 || nextBlock[lv]==5 || nextBlock[lv]==6) num_r=2;
    
    for(int r = 0; r < num_r; r++) {
        for(int j=-1; j<WIDTH-1;j++){
            while(CheckToMove(root->field, nextBlock[lv], r,++tempY, j)==1);
            tempY--;
            h[j]=(tempY);
        ...
        for(tempX = -1; tempX < WIDTH-1; tempX++) {
            ...
            //child= newChild(root);
            tempY = 0;
            while(CheckToMove(child->field, nextBlock[(child->lv)-1], r,++tempY, tempX)==1);
            tempY--;
            //tempY=h[tempX];
            child->score += AddBlockToField(child->field, nextBlock[(child->lv)-1], r, tempY,tempX);
            child->score += (DeleteLine(child->field));
            ...
            if(child->lv < MAX_LEVEL) {
                child->score += modified_recommend(child,child->lv);
            }
            if(max_score < child->score) {
                if(root->lv == 0) {
                    recommendR = r;
...
void print_result(){
    ...
    DrawBox(19,WIDTH+4,6,24);
    
    move(20,WIDTH+5);
    printw("play time: %lf", duration);
    
    move(21,WIDTH+5);
    printw("score(t) : %d", score);
    
    move(22,WIDTH+5);
    printw("time(t) : %lf", duration_t);
    
    move(23,WIDTH+5);
    printw("space(t) : %d", space);
   
    move(24,WIDTH+5);
    if(duration_t !=0)printw("score(t)/time(t) : %d ", score/(int)duration_t);
  
    move(25,WIDTH+5);
    if(space !=0 ) printw("score(t)/space(t) : %f", (float)score/(float)space);
